...
```{r plot_MiSeq_reads_by_sample_type, fig.height=5, fig.width=12}
...
```
```{r PCoA - Lung Samples v. Sampling Controls - Raw Counts}
#extract samples of interest with specified strings - returns vector of strings and NAs
extract.lungsamp.sampctrls <- str_extract(rownames(otu_good), pattern="_BAL_|_Lung_|PBS|CLEAN|USED|Homog")
...
#subset otu counts 
bray_df_raw <- otu_good[extract.lungsamp.sampctrls.good, ] 
...
# First step is to calculate a distance matrix. 
# Here we use Bray-Curtis distance metric
dist <- vegdist(bray_df_raw,  method = "bray")
PCOA <- pcoa(dist)
...
```
```{r PCoA - Lung Samples v. Isolation Controls - Raw Counts}
#extract samples of interest with specified strings - returns vector of strings and NAs
extract.lungsamp.isoctrls <- str_extract(rownames(otu_good), pattern="_BAL_|_Lung_|_IsoCtrl_|AE")
...
#subset otu counts 
bray_df_raw <- otu_good[extract.lungsamp.isoctrls.good, ] 
...
geom_label(aes(x=x_coord, y=y_coord, label=Sample_Type2, color=Sample_Type2), size=3, show.legend = FALSE) + # centroid information
  ...
  scale_y_continuous(limits = c(-0.6, 0.6), breaks = seq(-0.5, 0.5, 0.5)) +
  labs(
    x = "\nAxis 1 (13.3% of variance explained)",
    y = "Axis 2  (10.0% of variance explained)\n"
  ) + scale_color_manual(values=pca_pal)
...
#subset otu counts 
bray_df_raw <- otu_good[extract.lungsamp.seqctrls.good, ] 
...
geom_label(aes(x=x_coord, y=y_coord, label=Sample_Type2, color=Sample_Type2), size=3, show.legend = FALSE) + # centroid information
  ...
  scale_y_continuous(limits = c(-0.6, 0.6), breaks = seq(-0.5, 0.5, 0.5)) +
  labs(
    x = "\nAxis 1 (8.6% of variance explained)",
    y = "Axis 2  (6.7% of variance explained)\n"
  ) + scale_color_manual(values=pca_pal)
...
```
```{r PCoA - Lung Samples v. Tongue - Raw Counts}
#extract samples of interest with specified strings - returns vector of strings and NAs
extract.tong.lungsamp <- str_extract(rownames(otu_good), pattern="Tong|_BAL_|_Lung_")
...
#subset otu counts 
bray_df_raw <- otu_good[extract.tong.lungsamp.good, ] 
...
otu.df.tong.lungsamp <- otu.df.tong.lungsamp %>% dplyr::select(Sample:Shannon)
...
#subset otu counts 
bray_df_raw <- otu_good[extract.tong.lungsamp.allnegctrls.good, ] 
...
otu.df.tong.lungsamp.allnegctrls$PCA_Groups <- as.factor(otu.df.tong.lungsamp.allnegctrls$PCA_Groups)
otu.df.tong.lungsamp.allnegctrls <- otu.df.tong.lungsamp.allnegctrls %>% dplyr::select(Sample:Shannon)
