...
#pdf(file = "Raw_Read_Count.pdf", height = 5, width = 12, useDingbats = FALSE)
...
samples_below_100_read_threshold <- dplyr::filter(otu_df_plot_reads, Total_Reads <= 100) %>% 
                                    dplyr::select(Sample, Organ, Total_Reads)

kable(samples_below_100_read_threshold)

#create vector to filter samples with low read counts out 
samples.to.exclude <- samples_below_100_read_threshold$Sample %>% as.vector()
...
bray_df_perc <- otu.df.lungsamp.sampctrls %>% 
  ...
  dplyr::select(-c(Sample_Type:Gene_16S_copies_per_mL))
...
colnames(rarefy_agg) <- c("Sample", "Unique_Otus_per_100_reads")
...
rarefy_agg_summary  <- rarefy_agg %>% 
                       group_by(Sample_Type2) %>%
                       summarize(Mean_Species = mean(Unique_Otus_per_100_reads), 
                                 SEM = sqrt(var(Unique_Otus_per_100_reads)/length(Unique_Otus_per_100_reads)))
...
ggplot(rarefy_agg_summary, aes(x = Sample_Type2, y = Mean_Species, fill = Sample_Type2)) + # This brings in the aggregated dataframe and assigns columns to various plot elements
  ...
  geom_point(data = rarefy_agg, aes(x = Sample_Type2, y = Unique_Otus_per_100_reads ), width = .2, show.legend = FALSE) + # Use specimen level data, map columns to scatterplot attributes
...
otu_df  <- left_join(otu_df, dplyr::select(rarefy_agg, Sample, Unique_Otus_per_100_reads)) %>%
            dplyr::select(Sample:Gene_16S_copies_per_mL, Unique_Otus_per_100_reads, everything())

tukey_otu_df <- filter(otu_df, Sample_Type!="Mock" & Sample_Type!="Cecum" & Sample_Type!="Tongue")

TukeyHSD(aov(tukey_otu_df[,"Unique_Otus_per_100_reads"] ~ tukey_otu_df[, "Alpha_Div_Groups"]))
...
dim(sample_df) #[1] 84  2
comb_all <- combinations(n = 84, r = 2, v = sample_df$Sample) %>% as.data.frame()
